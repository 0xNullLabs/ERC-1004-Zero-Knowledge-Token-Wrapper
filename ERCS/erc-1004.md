...
A **ZWToken** is a wrapper token over an existing token (ERC-20, ERC-721, ERC-1155) that adds a _commitment-based privacy layer_.
...
- Transferring ZWToken: transferring ZWToken to any address, including provable burn addresses where ZWToken can be reminted.
...
- For commitment schemes maintained at the contract level, no commitment update is required, because a user holding ZWToken after calling deposit is certainly not a burn address.
